EESchema Schematic File Version 4
EELAYER 30 0
EELAYER END
$Descr A4 11693 8268
encoding utf-8
Sheet 1 1
Title ""
Date ""
Rev ""
Comp ""
Comment1 ""
Comment2 ""
Comment3 ""
Comment4 ""
$EndDescr
$Comp
L Ventilator-Splitter:ABPDJJ U3
U 1 1 5F37E909
P 9400 2600
F 0 "U3" H 9400 3265 50  0000 C CNN
F 1 "ABPDJJ" H 9400 3174 50  0000 C CNN
F 2 "" H 9400 2600 50  0001 C CNN
F 3 "https://www.mouser.com/datasheet/2/187/honeywell-sensing-basic-board-mount-pressure-abp-s-1662208.pdf" H 9400 2600 50  0001 C CNN
	1    9400 2600
	1    0    0    -1  
$EndComp
$Comp
L power:+5V #PWR0101
U 1 1 5F37F58C
P 2350 5000
F 0 "#PWR0101" H 2350 4850 50  0001 C CNN
F 1 "+5V" H 2365 5173 50  0000 C CNN
F 2 "" H 2350 5000 50  0001 C CNN
F 3 "" H 2350 5000 50  0001 C CNN
	1    2350 5000
	1    0    0    -1  
$EndComp
$Comp
L power:+3V3 #PWR0102
U 1 1 5F37F367
P 2150 5000
F 0 "#PWR0102" H 2150 4850 50  0001 C CNN
F 1 "+3V3" H 2165 5173 50  0000 C CNN
F 2 "" H 2150 5000 50  0001 C CNN
F 3 "" H 2150 5000 50  0001 C CNN
	1    2150 5000
	1    0    0    -1  
$EndComp
Wire Wire Line
	2150 5000 2200 5000
Wire Wire Line
	2200 5000 2200 5100
Wire Wire Line
	2350 5000 2300 5000
Wire Wire Line
	2300 5000 2300 5100
Wire Wire Line
	2000 7200 2100 7200
Connection ~ 2100 7200
Wire Wire Line
	2100 7200 2200 7200
Wire Wire Line
	2100 7200 2100 7250
Wire Wire Line
	1600 6700 1300 6700
Wire Wire Line
	1300 6600 1600 6600
Wire Wire Line
	1600 6800 1300 6800
Wire Wire Line
	1600 6500 1300 6500
Text Label 1300 6500 0    50   ~ 0
SS
Text Label 1300 6600 0    50   ~ 0
MOSI
Text Label 1300 6700 0    50   ~ 0
MISO
Text Label 1300 6800 0    50   ~ 0
SCK
$Comp
L Motor:Motor_Servo M2
U 1 1 5F37AABD
P 5700 5450
F 0 "M2" H 6032 5515 50  0000 L CNN
F 1 "Motor_Servo" H 6032 5424 50  0000 L CNN
F 2 "" H 5700 5260 50  0001 C CNN
F 3 "http://forums.parallax.com/uploads/attachments/46831/74481.png" H 5700 5260 50  0001 C CNN
	1    5700 5450
	1    0    0    -1  
$EndComp
$Comp
L Ventilator-Splitter:SM5420E U2
U 1 1 5F37CEB5
P 5700 6400
F 0 "U2" H 5700 7115 50  0000 C CNN
F 1 "SM5420E" H 5700 7024 50  0000 C CNN
F 2 "" H 5700 6400 50  0001 C CNN
F 3 "https://www.mouser.com/datasheet/2/589/SM5420E_40ds5468.02_1-1100662.pdf" H 5700 6400 50  0001 C CNN
	1    5700 6400
	1    0    0    -1  
$EndComp
$Comp
L power:+5V #PWR0103
U 1 1 5F39BF93
P 5350 5250
F 0 "#PWR0103" H 5350 5100 50  0001 C CNN
F 1 "+5V" H 5365 5423 50  0000 C CNN
F 2 "" H 5350 5250 50  0001 C CNN
F 3 "" H 5350 5250 50  0001 C CNN
	1    5350 5250
	1    0    0    -1  
$EndComp
$Comp
L power:GND #PWR0104
U 1 1 5F39C836
P 5350 5600
F 0 "#PWR0104" H 5350 5350 50  0001 C CNN
F 1 "GND" H 5355 5427 50  0000 C CNN
F 2 "" H 5350 5600 50  0001 C CNN
F 3 "" H 5350 5600 50  0001 C CNN
	1    5350 5600
	1    0    0    -1  
$EndComp
Wire Wire Line
	5350 5550 5350 5600
Wire Wire Line
	5400 5550 5350 5550
Wire Wire Line
	5400 5450 5350 5450
Wire Wire Line
	5350 5450 5350 5250
Wire Wire Line
	5400 5350 5150 5350
$Comp
L power:GND #PWR0105
U 1 1 5F380739
P 2100 7300
F 0 "#PWR0105" H 2100 7050 50  0001 C CNN
F 1 "GND" H 2105 7127 50  0000 C CNN
F 2 "" H 2100 7300 50  0001 C CNN
F 3 "" H 2100 7300 50  0001 C CNN
	1    2100 7300
	1    0    0    -1  
$EndComp
$Comp
L power:GND #PWR0106
U 1 1 5F3A152D
P 5300 6300
F 0 "#PWR0106" H 5300 6050 50  0001 C CNN
F 1 "GND" H 5305 6127 50  0000 C CNN
F 2 "" H 5300 6300 50  0001 C CNN
F 3 "" H 5300 6300 50  0001 C CNN
	1    5300 6300
	1    0    0    -1  
$EndComp
Wire Wire Line
	6050 5950 6100 5950
Wire Wire Line
	6100 5950 6100 5900
Wire Wire Line
	5350 6250 5300 6250
Wire Wire Line
	5300 6250 5300 6300
Wire Wire Line
	5350 6050 5150 6050
Wire Wire Line
	6050 6150 6250 6150
$Comp
L Motor:Motor_Servo M1
U 1 1 5F3A5562
P 5800 2450
F 0 "M1" H 6132 2515 50  0000 L CNN
F 1 "Motor_Servo" H 6132 2424 50  0000 L CNN
F 2 "" H 5800 2260 50  0001 C CNN
F 3 "http://forums.parallax.com/uploads/attachments/46831/74481.png" H 5800 2260 50  0001 C CNN
	1    5800 2450
	1    0    0    -1  
$EndComp
$Comp
L Ventilator-Splitter:SM5420E U1
U 1 1 5F3A5568
P 5800 3400
F 0 "U1" H 5800 4115 50  0000 C CNN
F 1 "SM5420E" H 5800 4024 50  0000 C CNN
F 2 "" H 5800 3400 50  0001 C CNN
F 3 "https://www.mouser.com/datasheet/2/589/SM5420E_40ds5468.02_1-1100662.pdf" H 5800 3400 50  0001 C CNN
	1    5800 3400
	1    0    0    -1  
$EndComp
$Comp
L power:+5V #PWR0107
U 1 1 5F3A556E
P 5450 2250
F 0 "#PWR0107" H 5450 2100 50  0001 C CNN
F 1 "+5V" H 5465 2423 50  0000 C CNN
F 2 "" H 5450 2250 50  0001 C CNN
F 3 "" H 5450 2250 50  0001 C CNN
	1    5450 2250
	1    0    0    -1  
$EndComp
$Comp
L power:GND #PWR0108
U 1 1 5F3A5574
P 5450 2600
F 0 "#PWR0108" H 5450 2350 50  0001 C CNN
F 1 "GND" H 5455 2427 50  0000 C CNN
F 2 "" H 5450 2600 50  0001 C CNN
F 3 "" H 5450 2600 50  0001 C CNN
	1    5450 2600
	1    0    0    -1  
$EndComp
Wire Wire Line
	5450 2550 5450 2600
Wire Wire Line
	5500 2550 5450 2550
Wire Wire Line
	5500 2450 5450 2450
Wire Wire Line
	5450 2450 5450 2250
Wire Wire Line
	5500 2350 5250 2350
$Comp
L power:GND #PWR0109
U 1 1 5F3A557F
P 5400 3300
F 0 "#PWR0109" H 5400 3050 50  0001 C CNN
F 1 "GND" H 5405 3127 50  0000 C CNN
F 2 "" H 5400 3300 50  0001 C CNN
F 3 "" H 5400 3300 50  0001 C CNN
	1    5400 3300
	1    0    0    -1  
$EndComp
Wire Wire Line
	6150 2950 6200 2950
Wire Wire Line
	6200 2950 6200 2900
Wire Wire Line
	5450 3250 5400 3250
Wire Wire Line
	5400 3250 5400 3300
Wire Wire Line
	5450 3050 5250 3050
Wire Wire Line
	6150 3150 6350 3150
Wire Wire Line
	8950 2300 8850 2300
Wire Wire Line
	8850 2300 8850 2100
Wire Wire Line
	8700 2200 8950 2200
Wire Wire Line
	8700 2400 8950 2400
Wire Wire Line
	9850 2200 10100 2200
Wire Wire Line
	9850 2300 10100 2300
NoConn ~ 9850 2400
Wire Wire Line
	8700 2800 8650 2800
Wire Wire Line
	8650 2800 8650 2750
Wire Wire Line
	8700 3500 8650 3500
Wire Wire Line
	8650 3500 8650 3550
Wire Wire Line
	8700 3300 8400 3300
Wire Wire Line
	8700 3200 8400 3200
Wire Wire Line
	8700 3100 8400 3100
$Comp
L power:+3V3 #PWR0110
U 1 1 5F3B1092
P 8850 2100
F 0 "#PWR0110" H 8850 1950 50  0001 C CNN
F 1 "+3V3" H 8865 2273 50  0000 C CNN
F 2 "" H 8850 2100 50  0001 C CNN
F 3 "" H 8850 2100 50  0001 C CNN
	1    8850 2100
	1    0    0    -1  
$EndComp
$Comp
L power:GND #PWR0111
U 1 1 5F3B1864
P 8650 3550
F 0 "#PWR0111" H 8650 3300 50  0001 C CNN
F 1 "GND" H 8655 3377 50  0000 C CNN
F 2 "" H 8650 3550 50  0001 C CNN
F 3 "" H 8650 3550 50  0001 C CNN
	1    8650 3550
	1    0    0    -1  
$EndComp
$Comp
L power:GND #PWR0112
U 1 1 5F3B1F52
P 8700 2200
F 0 "#PWR0112" H 8700 1950 50  0001 C CNN
F 1 "GND" V 8705 2072 50  0000 R CNN
F 2 "" H 8700 2200 50  0001 C CNN
F 3 "" H 8700 2200 50  0001 C CNN
	1    8700 2200
	0    1    1    0   
$EndComp
Text Label 10100 2200 2    50   ~ 0
SCK
Text Label 8700 2400 0    50   ~ 0
SS
Text Label 10100 2300 2    50   ~ 0
MISO
Text Label 8400 3300 0    50   ~ 0
SCK
Text Label 8400 3200 0    50   ~ 0
MOSI
$Comp
L Ventilator-Splitter:ST7735 DS1
U 1 1 5F3B52DB
P 9450 2700
F 0 "DS1" H 10028 2296 50  0000 L CNN
F 1 "ST7735" H 10028 2205 50  0000 L CNN
F 2 "" H 9450 2700 50  0001 C CNN
F 3 "" H 9450 2700 50  0001 C CNN
	1    9450 2700
	1    0    0    -1  
$EndComp
Text Label 8400 3100 0    50   ~ 0
SS
Wire Wire Line
	8400 3000 8700 3000
Wire Wire Line
	8400 2900 8700 2900
Wire Wire Line
	8700 3400 8650 3400
Wire Wire Line
	8650 3400 8650 2800
Connection ~ 8650 2800
$Comp
L power:+3V3 #PWR0113
U 1 1 5F3BAC72
P 8650 2750
F 0 "#PWR0113" H 8650 2600 50  0001 C CNN
F 1 "+3V3" H 8665 2923 50  0000 C CNN
F 2 "" H 8650 2750 50  0001 C CNN
F 3 "" H 8650 2750 50  0001 C CNN
	1    8650 2750
	1    0    0    -1  
$EndComp
Wire Wire Line
	1600 6400 1300 6400
Text Label 1300 6400 0    50   ~ 0
D9
Text Label 8400 2900 0    50   ~ 0
D9
Text Label 8400 3000 0    50   ~ 0
D8
Text Label 1300 6300 0    50   ~ 0
D8
Wire Wire Line
	1600 6300 1300 6300
$Comp
L power:+5V #PWR0114
U 1 1 5F3BD7C2
P 6200 2900
F 0 "#PWR0114" H 6200 2750 50  0001 C CNN
F 1 "+5V" H 6215 3073 50  0000 C CNN
F 2 "" H 6200 2900 50  0001 C CNN
F 3 "" H 6200 2900 50  0001 C CNN
	1    6200 2900
	1    0    0    -1  
$EndComp
$Comp
L power:+5V #PWR0115
U 1 1 5F3BDF48
P 6100 5900
F 0 "#PWR0115" H 6100 5750 50  0001 C CNN
F 1 "+5V" H 6115 6073 50  0000 C CNN
F 2 "" H 6100 5900 50  0001 C CNN
F 3 "" H 6100 5900 50  0001 C CNN
	1    6100 5900
	1    0    0    -1  
$EndComp
NoConn ~ 2000 5100
NoConn ~ 2600 5500
NoConn ~ 2600 5700
NoConn ~ 2600 5900
NoConn ~ 2600 6800
NoConn ~ 2600 6900
Wire Wire Line
	2600 6100 2850 6100
Wire Wire Line
	2600 6200 2850 6200
Wire Wire Line
	2600 6300 2850 6300
Wire Wire Line
	2600 6400 2850 6400
Text Label 2850 6100 2    50   ~ 0
A0
Text Label 2850 6200 2    50   ~ 0
A1
Text Label 2850 6300 2    50   ~ 0
A2
Text Label 2850 6400 2    50   ~ 0
A3
Text Label 5250 3050 0    50   ~ 0
A0
Text Label 6350 3150 2    50   ~ 0
A1
Text Label 5150 6050 0    50   ~ 0
A2
Text Label 6250 6150 2    50   ~ 0
A3
Wire Wire Line
	1600 6200 1300 6200
Wire Wire Line
	1600 6100 1300 6100
Text Label 1300 6200 0    50   ~ 0
D7
Text Label 1300 6100 0    50   ~ 0
D6
Text Label 3000 2800 2    50   ~ 0
D7
Text Label 5250 2350 0    50   ~ 0
D6
$Comp
L power:PWR_FLAG #FLG0101
U 1 1 5F3D589B
P 2100 7250
F 0 "#FLG0101" H 2100 7325 50  0001 C CNN
F 1 "PWR_FLAG" V 2100 7377 50  0000 L CNN
F 2 "" H 2100 7250 50  0001 C CNN
F 3 "~" H 2100 7250 50  0001 C CNN
	1    2100 7250
	0    -1   -1   0   
$EndComp
Connection ~ 2100 7250
Wire Wire Line
	2100 7250 2100 7300
Text Notes 1850 4600 0    100  ~ 20
Arduino\n
Wire Notes Line
	3950 1200 3950 7700
Wire Notes Line
	7600 1200 7600 6300
Text Notes 1850 950  0    200  ~ 40
Circuit Schematic of One Half of the Ventilator Splitter
Text Notes 5400 1650 0    100  ~ 20
PIP Valve
Text Notes 5300 4600 0    100  ~ 20
PEEP Valve
Text Notes 9000 1600 0    100  ~ 20
Spirometer
$Comp
L Ventilator-Splitter:HC-05 U4
U 1 1 5F579D98
P 2100 3300
F 0 "U4" H 2450 3400 50  0000 C CNN
F 1 "HC-05" H 2450 3300 50  0000 C CNN
F 2 "" H 2100 3300 50  0001 C CNN
F 3 "" H 2100 3300 50  0001 C CNN
	1    2100 3300
	1    0    0    -1  
$EndComp
$Comp
L power:+5V #PWR01
U 1 1 5F57A7E7
P 2100 2000
F 0 "#PWR01" H 2100 1850 50  0001 C CNN
F 1 "+5V" H 2115 2173 50  0000 C CNN
F 2 "" H 2100 2000 50  0001 C CNN
F 3 "" H 2100 2000 50  0001 C CNN
	1    2100 2000
	1    0    0    -1  
$EndComp
$Comp
L power:GND #PWR02
U 1 1 5F57B6FB
P 2100 3300
F 0 "#PWR02" H 2100 3050 50  0001 C CNN
F 1 "GND" H 2105 3127 50  0000 C CNN
F 2 "" H 2100 3300 50  0001 C CNN
F 3 "" H 2100 3300 50  0001 C CNN
	1    2100 3300
	1    0    0    -1  
$EndComp
Wire Wire Line
	1500 2600 1200 2600
Text Label 1200 2600 0    50   ~ 0
D1
Wire Wire Line
	1500 2700 1200 2700
Text Label 1200 2700 0    50   ~ 0
D0
Wire Wire Line
	1600 5600 1300 5600
Text Label 1300 5600 0    50   ~ 0
D1
Wire Wire Line
	1600 5500 1300 5500
Text Label 1300 5500 0    50   ~ 0
D0
Wire Wire Line
	2100 2100 2100 2000
Wire Wire Line
	2100 3200 2100 3300
Wire Wire Line
	2700 2500 3000 2500
Text Label 3000 2500 2    50   ~ 0
D2
Wire Wire Line
	2700 2600 3000 2600
Text Label 9950 5100 2    50   ~ 0
D3
Wire Wire Line
	2700 2700 3000 2700
Text Label 3000 2700 2    50   ~ 0
D4
Wire Wire Line
	2700 2800 3000 2800
Text Label 5150 5350 0    50   ~ 0
D5
Wire Wire Line
	1600 5700 1300 5700
Text Label 1300 5700 0    50   ~ 0
D2
Wire Wire Line
	1600 5800 1300 5800
Text Label 1300 5800 0    50   ~ 0
D3
Wire Wire Line
	1600 5900 1300 5900
Text Label 1300 5900 0    50   ~ 0
D4
Wire Wire Line
	1600 6000 1300 6000
Text Label 1300 6000 0    50   ~ 0
D5
$Comp
L MCU_Module:Arduino_UNO_R3 A1
U 1 1 5F376CFC
P 2100 6100
F 0 "A1" H 2550 5000 50  0000 C CNN
F 1 "Arduino_UNO_R3" H 2550 4900 50  0000 C CNN
F 2 "Module:Arduino_UNO_R3" H 2100 6100 50  0001 C CIN
F 3 "https://www.arduino.cc/en/Main/arduinoBoardUno" H 2100 6100 50  0001 C CNN
	1    2100 6100
	1    0    0    -1  
$EndComp
Text Notes 1750 1700 0    100  ~ 20
Bluetooth
$Comp
L Device:Buzzer BZ1
U 1 1 5F5A77D7
P 10300 5800
F 0 "BZ1" H 10452 5829 50  0000 L CNN
F 1 "Alarm_Buzzer" H 10452 5738 50  0000 L CNN
F 2 "" V 10275 5900 50  0001 C CNN
F 3 "~" V 10275 5900 50  0001 C CNN
	1    10300 5800
	1    0    0    -1  
$EndComp
$Comp
L Device:R R1
U 1 1 5F5A8DAE
P 8650 5850
F 0 "R1" H 8720 5896 50  0000 L CNN
F 1 "10k" H 8720 5805 50  0000 L CNN
F 2 "" V 8580 5850 50  0001 C CNN
F 3 "~" H 8650 5850 50  0001 C CNN
	1    8650 5850
	1    0    0    -1  
$EndComp
$Comp
L Switch:SW_Push SW1
U 1 1 5F5A800E
P 8650 5300
F 0 "SW1" V 8604 5448 50  0000 L CNN
F 1 "Alarm_Switch" V 8695 5448 50  0000 L CNN
F 2 "" H 8650 5500 50  0001 C CNN
F 3 "~" H 8650 5500 50  0001 C CNN
	1    8650 5300
	0    1    1    0   
$EndComp
Wire Wire Line
	2600 6500 2850 6500
Wire Wire Line
	2600 6600 2850 6600
Text Label 2850 6500 2    50   ~ 0
A4
Text Label 2850 6600 2    50   ~ 0
A5
Wire Wire Line
	8400 5550 8650 5550
Wire Wire Line
	9950 5100 10200 5100
Text Label 8400 5550 2    50   ~ 0
A4
Text Label 3000 2600 2    50   ~ 0
A5
Wire Wire Line
	8650 5500 8650 5550
Wire Wire Line
	8650 6000 8650 6100
$Comp
L power:GND #PWR03
U 1 1 5F5CC3F3
P 8650 6100
F 0 "#PWR03" H 8650 5850 50  0001 C CNN
F 1 "GND" H 8655 5927 50  0000 C CNN
F 2 "" H 8650 6100 50  0001 C CNN
F 3 "" H 8650 6100 50  0001 C CNN
	1    8650 6100
	1    0    0    -1  
$EndComp
Text Notes 8900 4600 0    100  ~ 20
Alarm System
Wire Notes Line
	600  4100 11000 4100
$Comp
L power:GND #PWR04
U 1 1 5F5CC6D4
P 10200 6100
F 0 "#PWR04" H 10200 5850 50  0001 C CNN
F 1 "GND" H 10205 5927 50  0000 C CNN
F 2 "" H 10200 6100 50  0001 C CNN
F 3 "" H 10200 6100 50  0001 C CNN
	1    10200 6100
	1    0    0    -1  
$EndComp
Wire Wire Line
	10200 5900 10200 6100
$Comp
L Device:R R2
U 1 1 5F575512
P 10200 5300
F 0 "R2" H 10270 5346 50  0000 L CNN
F 1 "100" H 10270 5255 50  0000 L CNN
F 2 "" V 10130 5300 50  0001 C CNN
F 3 "~" H 10200 5300 50  0001 C CNN
	1    10200 5300
	1    0    0    -1  
$EndComp
Wire Wire Line
	10200 5100 10200 5150
Wire Wire Line
	10200 5450 10200 5700
Connection ~ 8650 5550
Wire Wire Line
	8650 5550 8650 5700
$Comp
L power:+3V3 #PWR?
U 1 1 5F56E1A1
P 8650 5000
F 0 "#PWR?" H 8650 4850 50  0001 C CNN
F 1 "+3V3" H 8665 5173 50  0000 C CNN
F 2 "" H 8650 5000 50  0001 C CNN
F 3 "" H 8650 5000 50  0001 C CNN
	1    8650 5000
	1    0    0    -1  
$EndComp
Wire Wire Line
	8650 5000 8650 5100
$EndSCHEMATC
